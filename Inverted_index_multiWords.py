...
# Take more than one word from the user
words = input("Enter the words here separated by a space: ")
inputWords = words.split()

# For every word we just entered we will searc 
for word in inputWords:
    ...
    for i in range(len(fileNames)):
        ...
        # Search for the word in the document and add its index to the list if found
        if word in words:
            indices.append(i+1)
            
    # Print the indices for the current word
    if len(indices) > 0:
        print(f"The word '{word}' is in documents:", indices)
    else:
        print(f"The word '{word}' is not in any document.")